...
public class LandscapeDisplay extends JFrame
{
		...
		{
				// setup the window
				super("BlackJack ~ Carlson Productions");
		...
		private class LandscapePanel extends JPanel
		{
				...
				public void paintComponent(Graphics g)
				{
						...
						// draw all the agents in my specified order
						List<Cell> agents = scape.getAgents();
								for (Cell agent: agents)
								{
										if(agent instanceof Asthetics){
											agent.draw(g, 0, 0, scale);
										}
								}		
						for (Cell agent: agents)
								{
										if(agent instanceof Card || agent instanceof DealerHand){
											agent.draw(g, 0, 0, scale);
										}
								}
